...
if __name__ == '__main__':
    ...
    bag2 = path2 + "KF_GINS_Navresult_trans.bag"  # 由 12.nav_to_trans_bag.py 生成（tf2_msgs/TFMessage，话题 /tf）
    # 由 18.nav_mems_yaw.py 生成（sensor_msgs/Imu，话题 /mems_radyaw）
    bag3 = path2 + "IMU" + str(data["bag_name"]) + "_mems_radyaw.bag"

    # ===================== 配置区（修改为你的实际信息）=====================
    INPUT_BAGS = [bag1, bag2, bag3]  # 待合并的三个 bag 文件路径
    OUTPUT_BAG = path2 + str(data['bag_name']) + "_mems_radyaw.bag"
    # 含 KF_GINS_Navresult_trans.bag 中的 /tf（world -> base_link）
    TARGET_TOPICS = [
        "/FOGimuRad",
        "/MEMSimuRad",
        "/fix",
